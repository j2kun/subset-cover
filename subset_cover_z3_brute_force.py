...
def construct_problem_brute_force(num_elements, choice_set_size, hit_set_size):
    '''Construct the problem in such a way that all choice sets are enumerated.'''
    ...
    solver.add(sum(choice_counters) == 6)
    ...
    print(f"Chose {len(chosen_sets)} sets: {chosen_sets}")
    print(f"Hit {len(actual_hit_sets)}/{max_hits} sets: {actual_hit_sets}")

if __name__ == "__main__":
    construct_problem_brute_force(7, 3, 2)
