...
def construct_problem(num_elements, choice_set_size, hit_set_size, num_choice_sets):
    '''Build the problem in a way that avoids brute-force enumerating all choice sets.'''
    ...
    memberships = [
        ...
        for element, choice_set_index in
        product(elements, choice_sets)
    ]

    memberships_lookup = dict(
        ((mem.choice_set, mem.element), mem)
    ...
    choice_set_size_constraints = [
        sum(mem.variable for mem in memberships) == choice_set_size
    ...
    for choice_set in choice_sets:
        ...
        for membership_subset in combinations(mems, hit_set_size):
            hit_set = hit_sets[tuple(sorted([mem.element for mem in membership_subset]))]
            implications.append(
                Implies(
    ...
    for hit_set in hit_sets.values():
        ...
        for choice_set in choice_sets:
            mems = [memberships_lookup[(choice_set, elt)] for elt in hit_set.elements]
    ...
    print(f"Starting solve, checking if {num_choice_sets} choice sets is enough.")
    result = solver.check()
    ...
    for choice_set, mems in memberships_by_choice_set.items():
        choice_set_members = list(sorted([
            mem.element for mem in mems
            if model.evaluate(mem.variable).as_long() > 0
        ]))
        realized_choice_sets.append(choice_set_members)

    print(f"Chose {len(realized_choice_sets)} sets: {realized_choice_sets}")
    return realized_choice_sets
...
if __name__ == "__main__":
    ...
    num_sets = 1
    while True:
        result = construct_problem(7, 3, 2, num_sets)
